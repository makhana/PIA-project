...
export class ReviewComponent implements OnInit {
  ...
  submit() {
    if (!this.clientComment) {
      if (this.rating == 0 || this.comment == "") {
        ...
        this.clientService.addComment(this.username, this.request.agency, this.comment, this.rating).subscribe(resp => {
      ...
      if (this.rating == 0 && this.comment == "") {
        ...
        this.clientService.updateComment(this.username, this.request.agency, this.comment, this.rating).subscribe(resp => {
          console.log(resp['message']);
          this.ngOnInit();
        })
      }
  ...
  delete(){
    this.clientService.deleteComment(this.clientComment.agency, this.clientComment.client).subscribe(resp => {
      console.log(resp['message']);
      this.ngOnInit();
    })
  }
